    ...
    if (ImGui::Begin("Renderer", &w_renderer, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
    {
      ...
      if (ImGui::IsMouseClicked(ImGuiMouseButton_Left))
      {
        if (pixelCoord.x >= 0 && pixelCoord.x < rendWidth && pixelCoord.y >= 0 && pixelCoord.y < rendHeight)
        {
          ...
          std::function<std::optional<std::tuple<const Quadblock, const glm::vec3>>(int, int, std::vector<Quadblock>&, unsigned)> check = [](int pixelCoordX, int pixelCoordY, std::vector<Quadblock>& qbs, unsigned index)
            {
          ...
          static auto lastClickedLoc = pixelCoord;
          static int indenticalClickTimes = -1;
 
          if (lastClickedLoc.x == pixelCoord.x && lastClickedLoc.y == pixelCoord.y)
          {
            indenticalClickTimes++;
          }
          else
          {
            lastClickedLoc = pixelCoord;
            indenticalClickTimes = 0;
          }
 
          std::optional<std::tuple<const Quadblock, const glm::vec3>> collidedQB = check(pixelCoord.x, pixelCoord.y, m_quadblocks, indenticalClickTimes);
